...
const EditAdminUserPage = () => {
  ...
  useEffect(() => {
    if (edit) {
      const curr = decodeURIComponent(edit!);
      const currUser = userMock.find((user) => user.name === curr);
      ...
      if (currUser) {
        ...
        setUser({
          ...
          id: authState.user?.id as string,
          password: "",
          phone: "",
          role: "",
        });
  ...
  const onSave = () => {
    const index = userMock.findIndex((usr) => usr.name === user.name);
    ...
    <div className=" px-4 pb-[100px]">
      <DashboardMiniNav
        ...
        navItemsInOrder={[
          ...
          {
            name: `Edit - ${user.name}`,
            link: `/admin/user/edit/${user.name}`,
          },
        ]}
      />
